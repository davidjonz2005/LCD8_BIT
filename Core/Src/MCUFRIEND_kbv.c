...
int16_t
WIDTH=240,          ///< This is the 'raw' display width - never changes
HEIGHT=320; 
...
uint16_t _lcd_ID, _lcd_rev, _lcd_madctl, _lcd_drivOut, _MC, _MP, _MW, _SC, _EC, _SP, _EP, rotation;
...
void setColor2(uint8_t r, uint8_t g, uint8_t b) { setColor1(setrgb(r, g, b)); }
void setColor1(uint16_t c) { setTextColor(_fcolor = c, _bcolor); }

void clrScr() { fillScreen(0x0000); }
void fillRect1(int x1, int y1, int x2, int y2)
{
    ...
    fillRect(x1, y1, w, h, _fcolor);
}
void setTextSize(int sz)
{
    uint8_t s_x = gfxFont == NULL ? sz : 1;
    uint8_t s_y = gfxFont == NULL ? sz : 1;
    ...
    if (gfxFont)
    {
        ...
        { // Not a carriage return; is normal char
            ...
            {                                         // Char present in this font?
                GFXglyph *glyph = gfxFont->glyph + c; //pgm_read_glyph_ptr(gfxFont, c - first);
...
void InitLCD(uint8_t orientation)
{
    ...
    if (ID == 0xD3D3)
        ID = 0x9090; //write-only controller HX8357-D
...
void MCUFRIEND_kbv(int CS, int RS, int WR, int RD, int _RST)
{
    _width = 240;
    _height = 320;
    //_CS=CS;
...
uint16_t readID(void)
{
    ...
    uint8_t msb;
    idnum = readReg(0, 0); //forces a reset() if called before begin()
    if (idnum == 0x5408)   //the SPFD5408 fails the 0xD3D3 test.
        return 0x5408;
    if (idnum == 0x5420) //the SPFD5420 fails the 0xD3D3 test.
        return 0x5420;
    if (idnum == 0x8989) //SSD1289 is always 8989
        return 0x1289;
    idnum = readReg(0x67, 0); //HX8347-A
    if (idnum == 0x4747)
        return 0x8347;
    //#if defined(SUPPORT_1963) && USING_16BIT_BUS
    idnum = readReg32(0xA1); //SSD1963: [01 57 61 01]
    if (idnum == 0x6101)
        return 0x1963;
    if (idnum == 0xFFFF) //R61526: [xx FF FF FF]
        return 0x1526; //subsequent begin() enables Command Access
                       //    if (idnum == 0xFF00)          //R61520: [xx FF FF 00]
                       //        return 0x1520;          //subsequent begin() enables Command Access
                       //#endif
    idnum = readReg40(0xBF);
    if (idnum == 0x8357) //HX8357B: [xx 01 62 83 57 FF]
        return 0x8357;
    if (idnum == 0x9481) //ILI9481: [xx 02 04 94 81 FF]
        return 0x9481;
    if (idnum == 0x1511) //?R61511: [xx 02 04 15 11] not tested yet
        return 0x1511;
    if (idnum == 0x1520) //?R61520: [xx 01 22 15 20]
        return 0x1520;
    if (idnum == 0x1526) //?R61526: [xx 01 22 15 26]
        return 0x1526;
    if (idnum == 0x1581) //R61581:  [xx 01 22 15 81]
        return 0x1581;
    if (idnum == 0x1400) //?RM68140:[xx FF 68 14 00] not tested yet
        return 0x6814;
    idnum = readReg32(0xD4);
    if (idnum == 0x5310) //NT35310: [xx 01 53 10]
        return 0x5310;
    idnum = readReg32(0xD7);
    if (idnum == 0x8031) //weird unknown from BangGood [xx 20 80 31] PrinceCharles
        return 0x8031;
    idnum = readReg40(0xEF); //ILI9327: [xx 02 04 93 27 FF]
    if (idnum == 0x9327)
        return 0x9327;
    idnum = readReg32(0xFE) >> 8; //weird unknown from BangGood [04 20 53]
    if (idnum == 0x2053)
        return 0x2053;
    uint32_t ret32 = readReg32(0x04);
    msb = ret32 >> 16;
    idnum = ret32;
    //    if (msb = 0x38 && idnum == 0x8000) //unknown [xx 38 80 00] with D3 = 0x1602
    if (msb == 0x00 && idnum == 0x8000)
    ...
    //    if (msb == 0xFF && idnum == 0xFFFF) //R61526 [xx FF FF FF]
    //        return 0x1526;          //subsequent begin() enables Command Access
    if (idnum == 0x1526) //R61526 [xx 06 15 26] if I have written NVM
        return 0x1526; //subsequent begin() enables Command Access
    if (idnum == 0x89F0) //ST7735S: [xx 7C 89 F0]
        return 0x7735;
    if (idnum == 0x8552) //ST7789V: [xx 85 85 52]
        return 0x7789;
    if (idnum == 0xAC11) //?unknown [xx 61 AC 11]
        return 0xAC11;
    ret32 = readReg32(0xD3); //[xx 91 63 00]
    idnum = ret32 >> 8;
    if (idnum == 0x9163)
        return idnum;
    idnum = readReg32(0xD3); //for ILI9488, 9486, 9340, 9341
    msb = idnum >> 8;
    if (msb == 0x93 || msb == 0x94 || msb == 0x98 || msb == 0x77 || msb == 0x16)
        return idnum; //0x9488, 9486, 9340, 9341, 7796
    if (idnum == 0x00D3 || idnum == 0xD3D3)
        return idnum;       //16-bit write-only bus
                          /*
...
void fillRectgfx(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
#if defined(SUPPORT_9488_555)
    if (is555)
...
void invertDisplaygfx(uint8_t i)
{
    uint8_t val;
...
*/
    ...
    common_1289:
        ...
        static const uint16_t SSD1289_regValues[] = {
            ...
            0x00B7,
            0x0001,
            0x2B3F, // setRotation() alters
            0x0002,
            0x0600, // B_C=1, EOR=1
            0x0010,
            0x0000,
            0x0011,
            0x6070, // setRotation() alters
            0x0005,
            0x0000,
            0x0006,
            0x0000,
    ...
    case 0x1580:
        ...
        static const uint16_t R61580_regValues[] = {
            //from MCHIP Graphics Lib drvTFT001.c
            ...
            2,
            0x0060,
            0xA700, // Driver Output Control
            0x0008,
            0x0808, // Display Control BP=8, FP=8
            0x0030,
            0x0111, // y control
            0x0031,
            0x2410, // y control
        ...
        static const uint16_t R61580_DEM240320C[] = {
            //from DEM 240320C TMH-PW-N
            0x00,
            0x0000,
            0x00,
            0x0000,
    ...
    case 0x1963:
        ...
        static const uint8_t SSD1963_800_regValues[] = {
            ...
            0x1E,
            0x02,
            0x54, //PLL multiplier, set PLL clock to 120M
            (0xE0),
            1,
            0x01, // PLL enable
            TFTLCD_DELAY8,
            ...
            0x03,
            0xFF,
            0xFF, //PLL setting for PCLK, depends on resolution
            (0xB0),
            7,
            0x24,
            0x00,
            ...
            2,
            0x07,
            0x01, //GPIO3=input, GPIO[2:0]=output
            (0xF0),
            1,
            0x03, //pixel data interface
            TFTLCD_DELAY8,
            1,
            0x28,
            0, //Display Off
        ...
        static const uint8_t SSD1963_800NEW_regValues[] = {
            ...
            0x00,
            0x0F,
            0x00, //HSYNC HT=928, HPS=46, HPW=48, LPS=15
            (0xB6),
            7,
            0x02,
            0x0D,
            ...
            0x10,
            0x00,
            0x08, //VSYNC VT=525, VPS=16, VPW=16, FPS=8
            (0xBA),
            1,
            0x0F, //GPIO[3:0] out 1
            (0xB8),
        ...
        static const uint8_t SSD1963_800ALT_regValues[] = {
            ...
            0x23,
            0x02,
            0x04, //PLL multiplier, set PLL clock to 120M
            (0xE0),
            1,
            0x01, // PLL enable
            TFTLCD_DELAY8,
            ...
            0x04,
            0x93,
            0xE0, //PLL setting for PCLK, depends on resolution
            (0xB0),
            7,
            0x00,
            0x00,
    ...
    case 0x4535:
        ...
        static const uint16_t LGDP4535_regValues[] = {
            0x0015,
            0x0030, // Set the internal vcore voltage
            0x009A,
            0x0010, // Start internal OSC
            0x0011,
            0x0020, // set SS and SM bit
            0x0010,
            0x3428, // set 1 line inversion
            0x0012,
            0x0002, // set GRAM write direction and BGR=1
            0x0013,
            0x1038, // Resize register
            TFTLCD_DELAY,
            40,
            0x0012,
            0x0012, // set the back porch and front porch
            TFTLCD_DELAY,
            40,
            0x0010,
            0x3420, // set non-display area refresh cycle ISC[3:0]
            0x0013,
            0x3045, // FMARK function
            TFTLCD_DELAY,
            70,
            0x0030,
            0x0000, // RGB interface setting
            0x0031,
            0x0402, // Frame marker Position
            0x0032,
            0x0307, // RGB interface polarity
            0x0033,
            0x0304, // SAP, BT[3:0], AP, DSTB, SLP, STB
            0x0034,
            0x0004, // DC1[2:0], DC0[2:0], VC[2:0]
            0x0035,
            0x0401, // VREG1OUT voltage
            0x0036,
            0x0707, // VDV[4:0] for VCOM amplitude
            0x0037,
            0x0305, // SAP, BT[3:0], AP, DSTB, SLP, STB
            0x0038,
            0x0610, // DC1[2:0], DC0[2:0], VC[2:0]
            0x0039,
            0x0610, // VREG1OUT voltage
            0x0001,
            0x0100, // VDV[4:0] for VCOM amplitude
            0x0002,
            0x0300, // VCM[4:0] for VCOMH
            0x0003,
            0x1030, // GRAM horizontal Address
            0x0008,
            0x0808, // GRAM Vertical Address
            0x000A,
            0x0008,
            0x0060,
    ...
    case 0x7783:
        ...
        static const uint16_t ST7781_regValues[] = {
            ...
            50,
            0x0010,
            0x14B0, // Power Control 1 SAP=1, BT=4, APE=1, AP=3
            TFTLCD_DELAY,
            10,
            0x0011,
            0x0007, // Power Control 2 VC=7
            TFTLCD_DELAY,
            10,
            0x0012,
            0x008E, // Power Control 3 VCIRE=1, VRH=14
            0x0013,
            0x0C00, // Power Control 4 VDV=12
            0x0029,
            0x0015, // NVM read data 2 VCM=21
            TFTLCD_DELAY,
            10,
            0x0030,
            ...
            0x0035,
            0x0203, // Gamma Control 6
            0x0036,
            0x0402, // Gamma Control 7
            0x0037,
            0x0000, // Gamma Control 8
            0x0038,
            0x0207, // Gamma Control 9
            0x0039,
            0x0000, // Gamma Control 10
            0x003C,
            0x0203, // Gamma Control 13
            0x003D,
            0x0403, // Gamma Control 14
            0x0060,
            0xA700, // Driver Output Control (R60h) .kbv was 0xa700
            0x0061,
            0x0001, // Driver Output Control (R61h)
            0x0090,
            0X0029, // Panel Interface Control 1 (R90h)
    ...
    case 0x0099: //HX8357-D matches datasheet
        ...
        static const uint8_t HX8357_99_regValues[] = {
            ...
            (0xB6),
            1,
            0x25, // SETCOM [4B 00] -2.5V+37*0.02V=-1.76V [-1.00V]
            (0xC0),
            6,
            0x50,
            0x50,
            ...
            0x3C,
            0x1E,
            0x08, // SETSTBA [73 50 00 3C C4 08]
            (0xB4),
            7,
            0x02,
            0x40,
            ...
            0x2A,
            0x0D,
            0x78, // SETCYC [02 40 00 2A 2A 0D 96]
#ifdef SUPPORT_8357D_GAMMA
...
#define ILI9225C_INVON 0x21
    ...
    case 0x9225:
        ...
        static const uint16_t ILI9225_regValues[] = {
            ...
            ILI9225_POWER_CTRL1,
            0x0000, // Set SAP,DSTB,STB
            ILI9225_POWER_CTRL2,
            0x0000, // Set APON,PON,AON,VCI1EN,VC
            ILI9225_POWER_CTRL3,
            0x0000, // Set BT,DC1,DC2,DC3
            ILI9225_POWER_CTRL4,
            0x0000, // Set GVDD
            ILI9225_POWER_CTRL5,
            0x0000, // Set VCOMH/VCOML voltage
            TFTLCD_DELAY,
            40,

            // Power-on sequence
            ILI9225_POWER_CTRL2,
            0x0018, // Set APON,PON,AON,VCI1EN,VC
            ILI9225_POWER_CTRL3,
            0x6121, // Set BT,DC1,DC2,DC3
            ILI9225_POWER_CTRL4,
            0x006F, // Set GVDD   /*007F 0088 */
            ILI9225_POWER_CTRL5,
            0x495F, // Set VCOMH/VCOML voltage
            ILI9225_POWER_CTRL1,
            0x0800, // Set SAP,DSTB,STB
            TFTLCD_DELAY,
            10,
            ILI9225_POWER_CTRL2,
            0x103B, // Set APON,PON,AON,VCI1EN,VC
            TFTLCD_DELAY,
            50,

            ILI9225_DRIVER_OUTPUT_CTRL,
            0x011C, // set the display line number and display direction
            ILI9225_LCD_AC_DRIVING_CTRL,
            0x0100, // set 1 line inversion
            ILI9225_ENTRY_MODE,
            ...
            0x1100, // set the clocks number per line
            ILI9225_INTERFACE_CTRL,
            0x0000, // CPU interface
            ILI9225_OSC_CTRL,
            0x0D01, // Set Osc  /*0e01*/
            ILI9225_VCI_RECYCLING,
            0x0020, // Set VCI recycling
    ...
    common_9341:
        _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS | READ_24BITS;
        static const uint8_t ILI9341_regValues_2_4[] = {
            // BOE 2.4"
            0xF6,
            3,
            0x01,
    ...
    case 0x9342:
        ...
        static const uint8_t ILI9342_regValues_CPT24[] = {
            //CPT 2.4"
            (0xB9),
            3,
            0xFF,
        ...
        static const uint8_t ILI9342_regValues_Tianma23[] = {
            //Tianma 2.3"
            (0xB9),
            3,
            0xFF,
        ...
        static const uint8_t ILI9342_regValues_HSD23[] = {
            //HSD 2.3"
            (0xB9),
            3,
            0xFF,
    ...
    common_9481:
        ...
        static const uint8_t ILI9481_CPT29_regValues[] = {
            // 320x430
            0xB0,
            1,
            0x00,
        ...
        static const uint8_t ILI9481_PVI35_regValues[] = {
            // 320x480
            0xB0,
            1,
            0x00,
        ...
        static const uint8_t ILI9481_AUO317_regValues[] = {
            // 320x480
            0xB0,
            1,
            0x00,
        ...
        static const uint8_t ILI9481_RGB_regValues[] = {
            // 320x480
            0xB0, 1, 0x00,
...
#define GAMMA9486 4
    ...
    case 0x9488:
        _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS | READ_24BITS;
    common_9488:
        static const uint8_t ILI9488_regValues_max[] = {
            // Atmel MaxTouch
    ...
    case 0x9806:
        ...
        static const uint8_t ILI9806_regValues[] = {
            (0xFF),
            3,
            /* EXTC Command Set enable register*/ 0xFF,
            0x98,
            0x06,
            (0xBA),
            1,
            /* SPI Interface Setting*/ 0xE0,
            (0xBC),
            21,
            /* GIP 1*/ 0x03,
            0x0F,
            ...
            (0xF1),
            3,
            /* Panel Timing Control*/ 0x29,
            0x8A,
            0x07,
            (0xF2),
            4,
            /*Panel Timing Control*/ 0x40,
            0xD2,
            0x50,
            0x28,
            (0xC1),
            4,
            /* Power Control 2*/ 0x17,
            0x85,
            0x85,
            0x20,
            (0xE0),
